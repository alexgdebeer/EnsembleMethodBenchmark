...
function generate_pod_params(
    Nθ::Int,
    fname::AbstractString;
    N_r::Int=100
)

    xs = LinRange(0, 100, Nθ)

    l = 50 # TODO: think about this
    ds = (xs .- xs').^2
    C = exp.(-ds ./ (2 * l.^2)) + 1e-4I
    L = cholesky(Hermitian(C)).U 
    L_inv = inv(L)

    eigendecomp = @time eigen(C, sortby=(λ -> -λ))
    Λ_r = Diagonal(eigendecomp.values[1:N_r])
    V_r = eigendecomp.vectors[:, 1:N_r]

    h5write("data/$fname.h5", "L_inv", Matrix(L_inv))
    h5write("data/$fname.h5", "Λ_r", Matrix(Λ_r))
    h5write("data/$fname.h5", "V_r", V_r)

    return L_inv, Λ_r, V_r

end

function read_pod_params_data(
    fname::AbstractString
)
