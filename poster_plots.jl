...
PyPlot.rc("text"; Dict(
    :usetex => true, 
    Symbol("latex.preamble") => "\\usepackage[cm]{sfmath}")...
)

PyPlot.rc("font"; Dict(
    :family => "sans-serif", 
    Symbol("sans-serif") => "cm")...
)
...
function plot_forward_problem(g::Grid, well_centres, θ_t, u_t, d_obs, t_obs)
    ...
    _, axes = PyPlot.subplots(nrows=1, ncols=3, figsize=(6.0, 2.25))

    well_obs = get_well_obs(WELL_TO_PLOT)

    # Plot well positions
    for (i, c) ∈ zip(WELL_NUMS, well_centres)
        axes[1].scatter(c[1], c[2], s=10, facecolors="none", edgecolors="k")
        axes[1].text(c[1], c[2]+40, "Well $i", ha="center", fontsize=TEXT_SIZE)
    end
    axes[1].set_facecolor("lightskyblue")

    axes[2].pcolormesh(grid_f.xs, grid_f.xs, θ_t, cmap=CMAP_PERMS)

    axes[3].plot(ts, u_well, color="k", linestyle="--", linewidth=1)
    axes[3].scatter(t_obs, well_obs, color="k", s=5)

    axes[1].set_title("Well Locations", fontsize=TITLE_SIZE)
    axes[2].set_title("Reservoir Permeability", fontsize=TITLE_SIZE)
    axes[3].set_title("Pressure in Well 8", fontsize=TITLE_SIZE)
    ...
    PyPlot.tight_layout()
    PyPlot.savefig("plots/poster/forward_problem.pdf")
...
function plot_method_comparison(
    ...
    _, axes = PyPlot.subplots(nrows=3, ncols=4, figsize=(8, 6))
    ...
    fnames = [FNAME_PRIOR, FNAME_MCMC, FNAME_ENRML, FNAME_EKI]
    ...
    axes[1, 3].set_title("Ensemble Randomised\nMaximum Likelihood", fontsize=TITLE_SIZE)
    ...
    axes[3, 1].set_ylabel("Pressure in\nWell $(WELL_NUMS[4])", fontsize=TITLE_SIZE)
...
function generate_colourbars()
    ...
    PyPlot.savefig("plots/poster/colourbars.pdf")
...
function generate_legend()
    ...
    PyPlot.savefig("plots/poster/legend.pdf")
